...
# Tenta importar utils (se existir) para manter a barra lateral
try:
    import sys, os
...
st.set_page_config(
    page_title="Gestão de Prazos AIA", # <--- REMOVIDO "CCDR"
    page_icon="📅",
    layout="wide"
)

if HAS_UTILS: utils.sidebar_comum()
...
# ==========================================
# MOTOR DE FERIADOS E CÁLCULO (Mantém-se igual)
# ==========================================
# ... (Copie as funções get_easter_date, get_holidays_for_year, etc. do código anterior 
#      OU deixe estar se não quiser mexer na lógica matemática. 
#      O importante é a função create_pdf abaixo).

# VOU REPETIR APENAS A PARTE QUE MUDOU (O PDF e Textos):

# ... [Assuma que as funções de cálculo is_business_day, calculate_workflow, etc. estão aqui] ...
# (Para o código funcionar, mantenha as funções de cálculo originais aqui no meio)
...
def get_holidays_for_year(year):
    ...
    for m, d in fixed_dates: holidays.add(date(year, m, d))
...
def calculate_deadline_rigorous(start_date, target_business_days, suspensions, holidays_set, return_log=False):
    ...
    days_counted = 0
    log = []
    if return_log: log.append({"Data": current_date, "Dia Contado": 0, "Status": "Início"})
    ...
    while final_date.weekday() >= 5 or final_date in holidays_set: final_date += timedelta(days=1)
...
def calculate_workflow(start_date, suspensions, milestones_config, pea_date=None):
    ...
    results = []
    log_final = []
    steps = [("Data Reunião", milestones_config["reuniao"]), ("Limite Conformidade", milestones_config["conformidade"]),
             ("Envio PTF à AAIA", milestones_config["ptf"]), ("Audiência de Interessados", milestones_config["audiencia"]),
    ...
    for nome, dias in steps:
        ...
        if nome == "Limite Conformidade" and pea_date and suspensions:
            ...
            while check_date < pea_date:
                if is_business_day(check_date, holidays_set): days_spent += 1
                check_date += timedelta(days=1)
            remaining_days = max(0, dias - days_spent)
            ...
            if nome == "Limite Conformidade": conf_date_real = final_date
        results.append({"Etapa": nome, "Prazo Legal": f"{dias} dias úteis", "Data Prevista": final_date})
...
# ==========================================
# 5. GERADOR DE PDF (CORRIGIDO E NEUTRO)
# ==========================================
def create_pdf(project_name, typology, sector, regime, start_date, milestones, complementary, suspensions, total_susp, gantt_data):
    ...
    class PDF(FPDF):
        def header(self):
            ...
            self.set_text_color(80, 80, 80) # Cinzento neutro em vez de azul
            # ALTERAÇÃO AQUI: Texto Genérico
            self.cell(0, 10, 'CRONOGRAMA DE PROCEDIMENTO DE AIA', 0, 1, 'C')
    ...
    # ... (Resto do código do PDF igual ao original, apenas o Header mudou) ...
    
    # 1. Enquadramento
    pdf.set_fill_color(240, 240, 240) # Cinzento neutro
    ...
    if complementary:
        pdf.ln(5)
        pdf.set_font("Arial", "B", 11)
        pdf.cell(0, 8, "4. Prazos Complementares", 0, 1, 'L', 1)
        pdf.set_font("Arial", "", 9)
        pdf.set_font("Arial", "B", 9)
        pdf.cell(90, 8, "Etapa", 1, 0, 'L', 1)
...
st.title("Gestão de Prazos AIA") # <--- REMOVIDO "CCDR" e "Analista"
st.markdown("Cálculo de prazos de acordo com o **RJAIA** e **Simplex Ambiental**.")

if FPDF is None:
    st.error("⚠️ Aviso: A biblioteca 'fpdf' não está instalada. A geração de PDF não funcionará.")

# Dados legais (Simplificados para não depender de imports externos complexos se não quiser)
COMMON_LAWS = {"RJAIA (DL 151-B/2013)": "#", "Simplex (DL 11/2023)": "#"}
TIPOLOGIAS_INFO = {
    "Anexo I (AIA Obrigatória)": "Projetos listados no Anexo I.",
    "Anexo II (Análise Caso a Caso)": "Projetos sujeitos a verificação prévia.",
    "Alteração ou Ampliação": "Alterações a projetos existentes."
}
SPECIFIC_LAWS = {"Indústria": {}, "Energia": {}, "Infraestruturas": {}}

# --- SIDEBAR (Conteúdo Específico desta App) ---
# Nota: Como já temos a sidebar_comum, usamos st.sidebar para adicionar coisas ABAIXO dela.
with st.sidebar:
    st.markdown("---")
    st.header("📂 Dados do Projeto")
    proj_name = st.text_input("Nome do Projeto", "Novo Projeto")
    start_date = st.date_input("Data de Instrução", date.today())
    ...
    regime_option = st.radio("Prazo Global:", (150, 90), format_func=lambda x: f"{x} Dias Úteis")
    
    with st.expander("⚙️ Ajuste de Prazos"):
        if regime_option == 150:
            ...
            d_dia = st.number_input("Decisão Final", 150, disabled=True)
        else:
            d_reuniao = st.number_input("Reunião", 9)
            d_conf = st.number_input("Conformidade", 20)
            d_ptf = st.number_input("Envio PTF", 65)
            d_aud = st.number_input("Audiência", 70)
            d_dia = st.number_input("Decisão Final", 90, disabled=True)
            
        d_cp_duration = st.number_input("Duração CP", 30)
        d_visita = st.number_input("Visita Técnica", 15)
        d_setoriais = st.number_input("Pareceres Setoriais", 75 if regime_option==150 else 60)
    ...
    with st.form("add_susp"):
        c1, c2 = st.columns(2)
        ns = c1.date_input("Início")
        ne = c2.date_input("Fim")
        if st.form_submit_button("Adicionar"):
            st.session_state.suspensions_universal.append({'start': ns, 'end': ne})
            st.rerun()
...
milestones, complementary, total_susp, log_dia, gantt_data = calculate_workflow(
    start_date, st.session_state.suspensions_universal, milestones_config
)
final_date = milestones[-1]["Data Prevista"]

# DASHBOARD
c1, c2, c3 = st.columns(3)
c1.metric("Fim Previsto", final_date.strftime("%d/%m/%Y"))
c2.metric("Regime", f"{regime_option} dias")
c3.metric("Suspensão Total", f"{total_susp} dias")

tab1, tab2, tab3 = st.tabs(["Tabela Prazos", "Gráfico Gantt", "Exportar"])
...
with tab3:
    if st.button("Gerar PDF do Relatório"):
        pdf_bytes = create_pdf(
            proj_name, selected_typology, selected_sector, 
            f"Regime {regime_option} Dias", start_date, 
            milestones, complementary, st.session_state.suspensions_universal, 
            total_susp, gantt_data
        )
        if pdf_bytes:
            st.download_button("Download PDF", pdf_bytes, "cronograma_aia.pdf", "application/pdf")
